...
function cleanLinks(array $labels, array $urls): array
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        $links[] = [
            'label' => $label !== '' ? $label : parse_url($url, PHP_URL_HOST),
            'url' => $url,
        ];
    }
    return $links;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $socials = [
        'facebook' => cleanText($_POST['facebook'] ?? ''),
        'linkedin' => cleanText($_POST['linkedin'] ?? ''),
        'youtube' => cleanText($_POST['youtube'] ?? ''),
        'instagram' => cleanText($_POST['instagram'] ?? ''),
        'tiktok' => cleanText($_POST['tiktok'] ?? ''),
    ];
            ...
            <ul class="card-list">
                <?php foreach (array_reverse($cards) as $card): ?>
                    <li>
                        ...
                        <a href="card.php?id=<?php echo urlencode($card['id']); ?>">Open</a>
                    </li>
                <?php endforeach; ?>
            </ul>
        <?php endif; ?>
